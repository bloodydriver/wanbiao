$(function() {
    ...
    $(".w_cont-bt16").click(function() {
        ...
        for(var i=0;i<$(".addCart").length;i++) {
            // console.log($(".addCart .w_cont-bt7").eq(i).text().substr(1).split(",").join(""))
            cartCode+=`
            <li>
                <i class="check"></i>
                <p>
                    <img src="${$(".addCart .w_cont_img1").eq(i)[0].src}" alt="">
                </p>
                <div class="content">
                    <p>${$(".addCart .w_cont-bt8").eq(i).text()}</p>
                    <p>真皮 黑色</p>
                </div>
                <div class="control">
                    <span>￥<span class="price">${$(".addCart .w_cont-bt7").eq(i).text().split(" ").join("").substr(1).split(",").join("")}</span></span>
                    <div>
                        <button class="sub" type="button">-</button>
                        <input class="number" type="text" value="1" disabled>
                        <button class="add" type="button">+</button>
                    </div>
                    <span class="priceAll" style="font-weight: 700;">￥${$(".addCart .w_cont-bt7").eq(i).text().split(" ").join("").substr(1).split(",").join("")}</span>
